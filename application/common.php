...
function update_pay_status($order_sn, $transaction_id = '')
{
    if (stripos($order_sn, 'recharge') !== false) {
        ...
        if ($user['is_usercenter'] == 1) {
            ...
            $rs = curl_post('http://202.61.87.33:8066/tools/zxajax.ashx?action=WZ_user_buy', array('UserNo' => $user['member_name'], 'orderje' => $order['integral']));

            $jifen = json_decode($rs);

            if ($jifen->status != 1) {
                $data['member_name'] = $user['member_name'];
                $data['credit'] = $order['integral'];
                $data['addtime'] = time();
                M('submit_errorlog')->add($data);
            }
